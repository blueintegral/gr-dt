...
# Title: Simple Trx 2
# Generated: Mon Oct 15 07:12:19 2012
##################################################
...
class simple_trx_2(grc_wxgui.top_block_gui):
...
if __name__ == '__main__':
	...
	tb = simple_trx_2(rate=options.rate, tx_gain=options.tx_gain, samp_per_sym=options.samp_per_sym, freq=options.freq, dest_addr=options.dest_addr, arq_timeout=options.arq_timeout, max_arq_attempts=options.max_arq_attempts, rx_freq=options.rx_freq, rx_gain=options.rx_gain, args=options.args, ampl=options.ampl, port=options.port, radio_addr=options.radio_addr, rx_ant=options.rx_ant)
	tb.Run(True)
